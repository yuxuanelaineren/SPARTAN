...
# Read the file
merged_df = pd.read_excel(os.path.join(out_dir, 'C360_CEDS_noLUO_Sim_vs_SPARTAN_other_BC_2019_Summary.xlsx'))


# Create a 2D histogram to divide the area into squares and count data points in each square
hist, xedges, yedges = np.histogram2d(merged_df['obs'], merged_df['sim'], bins=200)
...
# plt.xlim([merged_df['obs'].min()-0.5, 12])
# plt.xlim([merged_df['obs'].min()-0.5, 12])
plt.xlim([0.001, 20])
plt.ylim([0.001, 20])
plt.xticks([0.001, 4, 8, 12, 16], fontname='Arial', size=18)
plt.yticks([0.001, 4, 8, 12, 16], fontname='Arial', size=18)
...
plt.plot([merged_df['obs'].min(), merged_df['obs'].max()], [merged_df['obs'].min(), merged_df['obs'].max()], color='grey', linestyle='--', linewidth=1)
...
mask = ~np.isnan(merged_df['obs']) & ~np.isnan(merged_df['sim'])
slope, intercept, r_value, p_value, std_err = stats.linregress(merged_df['obs'][mask], merged_df['sim'][mask])
...
if np.isnan(slope) or np.isnan(intercept) or np.isnan(r_value):
    ...
    sns.regplot(x='obs', y='sim', data=merged_df, scatter=False, ci=None, line_kws={'color': 'k', 'linestyle': '-', 'linewidth': 1})
...
plt.xlabel('obs Black Carbon Concentration (µg/m$^3$)', fontsize=18, color='black', fontname='Arial')
plt.ylabel('sim Black Carbon Concentration (µg/m$^3$)', fontsize=18, color='black', fontname='Arial')

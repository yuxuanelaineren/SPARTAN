...
for mon in range(1, 13):
    ...
    # Print the results
    print(f'Simulated_{species} - Mean: {mean_sim:.2f}, SD: {sd_sim:.2f}, Max: {max_sim:.2f}')
    print(f'Observed_{species} - Mean: {mean_obs:.2f}, SD: {sd_obs:.2f}, Max: {max_obs:.2f}')
...
for mon in range(1, 2):
    ...
    ax.text(0.6, 0.15, 'Sim = {:.1f}'.format(global_mean_sim) + ' µg/m$^3$', fontsize=10, fontname='Arial',
            transform=ax.transAxes)
    ax.text(0.6, 0.08, 'Obs = {:.1f}'.format(global_mean_obs) + ' µg/m$^3$', fontsize=10, fontname='Arial',
            transform=ax.transAxes)
    ...
    plt.colorbar(im, label="BC concentrations (µg/m$^3$)", orientation="horizontal",

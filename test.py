...
x_range = [compr_df['FTIR_OM'].min(), compr_df['FTIR_OM'].max()]
# Create figure and axes objects
fig, ax = plt.subplots(figsize=(8, 6))

# Create scatter plot with white background, black border, and no grid
sns.set(font='Arial')
scatterplot = sns.scatterplot(x='FTIR_OM', y='Residual', data=compr_df, hue='city', palette=city_palette, s=20, alpha=1, edgecolor='k', style='city',  markers=city_marker)
...
x = compr_df['FTIR_OM']
y = compr_df['FTIR_OM']
plt.plot([compr_df['FTIR_OM'].min(), 50], [compr_df['FTIR_OM'].min(), 50], color='grey', linestyle='--', linewidth=1)

# Perform linear regression for all segments
mask = (compr_df['FTIR_OM'] >= x_range[0]) & (compr_df['FTIR_OM'] <= x_range[1])
slope, intercept, r_value, p_value, std_err = stats.linregress(compr_df['FTIR_OM'][mask], compr_df['Residual'][mask])
# Plot regression lines
sns.regplot(x='FTIR_OM', y='Residual', data=compr_df[mask],
...
# Show the plot
plt.tight_layout()
plt.savefig(out_dir + 'FTIR_OM_vs_Residual.svg', dpi=300)

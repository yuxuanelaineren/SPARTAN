...
# Initialize annual concentration
annual_conc = None

# Loop through each face
for face in range(6):
    # Initialize face-specific annual concentration
    face_annual_conc = None

    for mon in range(1, 12 + 1):  # Iterate through the 12 months
        print(f"Opening file for face {face}, month {mon}:")
        file_path = sim_dir + '{}.noLUO.CEDS01-vert.PM25.RH35.NOx.O3.{}{:02d}.MonMean.nc4'.format(cres, year, mon)
        print(file_path)

        with xr.open_dataset(file_path, engine='netcdf4') as sim_df:  # Open the monthly data file
            # Extract coordinates for the current face
            x = sim_df.corner_lons.isel(nf=face)
            y = sim_df.corner_lats.isel(nf=face)

            # Extract species concentration for the current face
            conc = sim_df[species].isel(nf=face).load()

            # Apply the land-sea mask (ensure mask is applied for the current face)
            conc = np.where(land_mask, conc, np.nan)

            # Accumulate the concentration data for the year (face-specific)
            if face_annual_conc is None:
                face_annual_conc = conc
            else:
                face_annual_conc += conc

        print("File closed.")

    # Calculate the annual average for the current face
    face_annual_conc /= 12
    face_annual_conc = face_annual_conc.squeeze()  # Squeeze to remove singleton dimensions if needed

    print(f"x shape: {x.shape}, y shape: {y.shape}, annual_conc shape: {face_annual_conc.shape}")

    # Plot the annual average data for the current face
    im = ax.pcolormesh(x, y, face_annual_conc, cmap=cmap, transform=ccrs.PlateCarree(), vmin=0, vmax=vmax)

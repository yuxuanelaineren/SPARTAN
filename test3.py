...
# annual_df = annual_df.loc[annual_df['obs'] <= 8]
...
legend = plt.legend(facecolor='white', bbox_to_anchor=(1.03, 0.50), loc='center left', fontsize=9)
...
plt.title(f'Sulfate Comparison: GCHP-v13.4.1 {cres.lower()} v.s. SPARTAN',
...
plt.xticks([0, 5, 10, 15], fontname='Arial', size=14)
plt.yticks([0, 5, 10, 15], fontname='Arial', size=14)
...
plt.text(0.70, 0.05, f'N = {num_points}', transform=scatterplot.transAxes, fontsize=14)
...
if np.isnan(slope) or np.isnan(intercept) or np.isnan(r_value):
    ...
    plt.text(0.70, 0.1, f"y = {slope:.2f}x {intercept_sign} {intercept_display:.2f}\n$r^2$ = {r_value ** 2:.2f}",
             transform=plt.gca().transAxes, fontsize=14)

plt.xlabel('SPARTAN Sulfate (µg/m$^3$)', fontsize=14, color='black', fontname='Arial')
plt.ylabel('GCHP Sulfate (µg/m$^3$)', fontsize=14, color='black', fontname='Arial')

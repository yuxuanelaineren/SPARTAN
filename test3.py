...
################################################################################################
# Combine FT-IR EC and GCHP dataset based on lat/lon
################################################################################################
...
for mon in range(1, 13):
    ...
    # Load the Data
    obs_df = pd.read_excel('/Volumes/rvmartin/Active/ren.yuxuan/BC_Comparison/SPARTAN_BC_old/BC_HIPS_FTIR_UV-Vis_SPARTAN_allYears.xlsx',
                           usecols=['FilterID', 'start_year', 'start_month', 'EC_FTIR_ug/m3_raw', 'Site', 'Country', 'City', 'Latitude', 'Longitude'])
...
with pd.ExcelWriter(out_dir + '{}_{}_{}_vs_FT-IR_{}_{}.xlsx'.format(cres, inventory, deposition, species, year), engine='openpyxl') as writer:

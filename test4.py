...
# Create separate masks for red and blue groups
red_mask = compr_df['color'] == 'red'
blue_mask = compr_df['color'] == 'blue'
print(compr_df['color'].unique())
city_palette = {'red': 'red', 'blue': 'blue', 'black': 'black', 'other': 'grey'}
...
plt.text(0.1, 0.9, f'y = {slope_red:.2f}x + {intercept_red:.2f}, $r^2$ = {r_value_red**2:.2f}', color='red', transform=ax.transAxes, fontsize=12)
...
plt.text(0.1, 0.85, f'y = {slope_blue:.2f}x + {intercept_blue:.2f}, $r^2$ = {r_value_blue**2:.2f}', color='blue', transform=ax.transAxes, fontsize=12)

# # Perform linear regression for the first segment
# mask_1 = (compr_df['obs'] >= x_range_1[0]) & (compr_df['obs'] <= x_range_2[1]) # One segment
# # mask_1 = (compr_df['obs'] >= x_range_1[0]) & (compr_df['obs'] <= x_range_2[1]) & (compr_df['city'] != 'Beijing') # Exclude Beijing
...
#          transform=scatterplot.transAxes, fontsize=18, color='k')
# # Add the number of data points
# num_points_1 = mask_1.sum()
# plt.text(0.6, 0.77, f'N = {num_points_1}', transform=scatterplot.transAxes, fontsize=18, color='k')
# plt.text(0.8, 0.03, f'{inventory}', transform=scatterplot.transAxes, fontsize=18, color='k')
